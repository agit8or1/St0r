...
export function ClientDetail() {
  ...
  const [installerMessage, setInstallerMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
  ...
  const handleDownloadInstaller = async (platform: 'windows' | 'linux') => {
    ...
    try {
      ...
      setInstallerMessage({
        type: 'error',
        text: error instanceof Error ? error.message : 'Failed to prepare download. Please try again.',
      });
    }
  };
        ...
        <div className="card bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20">
          ...
          {serverInfo && (
            <div className="mb-4 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
              ...
              <p className="text-xs text-green-700 dark:text-green-300 mt-1">
                ...
                <br />
                <strong>Linux:</strong> Downloads a preconfigured self-extracting installer. Run it with <span className="font-mono">sudo sh &lt;file&gt;.sh</span> — it installs the client and writes the server, port, name and auth key for you.
                <br />
          ...
          {installerMessage && (
            <div className={`mb-4 p-3 rounded-lg border ${
              installerMessage.type === 'success'
                ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800 text-blue-800 dark:text-blue-200'
                : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800 text-red-800 dark:text-red-200'
            }`}>
              <p className="text-sm break-words">{installerMessage.text}</p>
            </div>
          )}
